(ns vocational-education.actor
  "VocationalEducationActor — the ISCO-08 2320 independent vocational-
  education actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  training operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the EducationAdvisor can never directly
  commit a record or dispatch a robot action the
  VocationalEducationGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [vocational-education.advisor :as advisor]
            [vocational-education.governor :as governor]
            [vocational-education.store :as store]))

(defn build-graph
  "Build a compiled VocationalEducationActor graph. `store` implements
  `vocational-education.store/Store`. `advisor` implements
  `vocational-education.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:student-id (:student-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
