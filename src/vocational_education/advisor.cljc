(ns vocational-education.advisor
  "EducationAdvisor — proposes a vocational-education operation (teach
  support, assess, operate a power tool near a student, operate heavy
  equipment near a student) for a registered student. The advisor is
  ...
  and has no notion of student provenance or power-tool/heavy-
  equipment risk; `vocational-education.governor` is the independent
  system that decides whether the proposal may proceed, per the
  itonami actor pattern.

  A proposal is a map:
    {:op :teach-support|:assess|:operate-power-tool-near-student|:operate-heavy-equipment-near-student
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for student " (:student-id request))})
...
(def ^:private system-prompt
  "You are a vocational-education advisor. Given a training operation
